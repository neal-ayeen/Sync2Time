...
const rosterRows = [
  ...
  ['Charles Kith CaÃ±ete', 'Webinar Admin', 'Admin Department', 'Webinar preparation', 'Commission Based', null, null, 'M1nute-W2Fs'],
...
employeeDirectory.forEach((person, index) => {
  ...
  person.clockIn = person.scheduledStart === null ? 'â€”' : toClock(person.scheduledStart + [-3, 3, 11, -11][index % 4]);
  person.clockOut = isLive ? 'â€”' : person.scheduledEnd === null ? 'â€”' : toClock(person.scheduledEnd + [22, 7, -4, 18][index % 4]);
...
function rosterSource() {
  return usesSupabase() && supabaseProfiles.length ? supabaseProfiles.map(profileToPerson).filter(Boolean) : employeeDirectory;
}
...
function attendanceRecordFromSupabase(entry, index) {
  ...
  return {
    ...
    clockIn: clockIn ? formatClock(clockIn) : 'â€”',
    clockOut: clockOut ? formatClock(clockOut) : 'â€”',
    worked: formatDuration(seconds),
...
function render() {
  ...
  $('#timerHint').textContent = running ? `Clocked in at ${formatClock(running.start)} Â· keep going.` : 'Choose a task, then clock in.';
  ...
  $('#clockOutDisplay').textContent = state.clockOut ? formatClock(state.clockOut) : 'â€”';
...
function renderLiveTeam() {
  ...
  $('#liveTeamRows').innerHTML = online.map((person, index) => `<div class="live-team-row" role="button" tabindex="0" data-employee="${encodeURIComponent(person.name)}"><div class="person"><span class="person-avatar" style="background:${person.color}">${person.initials}</span><span>${person.name}<small>${person.role}</small></span></div><span class="task-cell"><i class="project-dot ${index % 2 ? 'blue' : 'coral'}"></i>${person.task}</span><span class="live-clock">${person.clockIn}</span><span class="live-clock">${person.worked}</span><span class="status active">â— Working</span></div>`).join('') || '<div class="empty-state">No employees are clocked in right now.</div>';
}

function toMinutes(time) {
  if (!time || time === 'â€”') return null;
...
function scheduleStatus(person) {
  ...
  if (person.scheduledStart === null) return person.status === 'clocked' ? { type: 'checked-in', label: 'Working Â· flexible shift', className: 'checked' } : { type: 'complete', label: 'Flexible shift', className: 'checked' };
...
function rateLabel(person) {
  ...
  return person.rate >= 1000 ? `â‚±${person.rate.toLocaleString()}/mo â†’ ${money(hourlyRate(person))}/hr` : `$${person.rate.toFixed(2)}/hr`;
}

function money(value) {
  return value === null || Number.isNaN(value) ? 'â€”' : `$${value.toFixed(2)}`;
}
...
function renderProjects() {
  ...
  $('#projectRows').innerHTML = rows.map(({ person, project }) => `<div class="project-row" role="button" tabindex="0" data-employee="${encodeURIComponent(person.name)}"><div class="person"><span class="person-avatar" style="background:${person.color}">${person.initials}</span><span>${person.name}<small>${person.role}</small></span></div><span class="project-tag">${project}</span><span>${person.task}</span><span class="schedule-shift">${person.schedule}</span><span class="status ${person.status === 'clocked' ? 'active' : 'complete'}">${person.status === 'clocked' ? 'â— Working' : 'Complete'}</span></div>`).join('') || '<div class="empty-state">No employees in this project category.</div>';
...
function renderManagedEmployees() {
  ...
  select.innerHTML = '<option value="">Select employee</option>' + managedEmployees.map(employee => `<option value="${employee.id}">${escapeHtml(employee.name)} â€” ${escapeHtml(employee.email)}</option>`).join('');
...
function renderTeamDirectory() {
  ...
  $('#teamRows').innerHTML = roster.map(person => {
    ...
    const passwordLine = access?.tempPassword ? `<small>${access.email} Â· Pass: ${access.tempPassword}</small>` : `<small>${person.department || 'Employee'}</small>`;
...
function renderHolidayPay() {
  ...
  $('#holidayMultiplier').textContent = regular ? '2Ã— regular-holiday pay' : '1Ã— standard pay';
...
function renderPayday() {
  ...
  $('#paydayMessage').textContent = days === 0 ? 'Payroll is due today.' : `${days} day${days === 1 ? '' : 's'} until payday Â· ${payday.getDate() === 20 ? 'Work from the 1st-15th' : 'Work from the 16th-end of month'}.`;
...
function renderLeaveCalendar() {
  renderCalendar(leaveCalendarCursor, '#leaveCalendarMonth', '#leaveCalendar', date => {
    ...
    return { className: request ? 'leave-booked' : '', note: request ? `${request.employeeName} Â· ${request.status}` : '' };
  });
}
...
async function refreshFxRate() {
  try {
    ...
    $('#fxRateLabel').textContent = 'Live PHPâ†’USD reference rate';
    $('#fxRateValue').textContent = `â‚±1.00 = $${phpUsdRate.toFixed(4)}`;
    $('#fxRateMeta').textContent = `Rate date: ${fxRateDate} Â· used for PHP monthly salaries.`;
  } catch {
    $('#fxRateLabel').textContent = 'Offline PHPâ†’USD reference rate';
    $('#fxRateValue').textContent = `â‚±1.00 = $${phpUsdRate.toFixed(4)}`;
    $('#fxRateMeta').textContent = `${fxRateDate === 'offline reference' ? 'Fallback estimate while offline' : 'Last saved rate: ' + fxRateDate} Â· reconnect to refresh.`;
  }
  renderScheduleWatch();
}
...
function approvalRow(request, type) {
  const details = type === 'leave' ? `${request.date} Â· ${request.reason}` : `${request.date} Â· ${request.clockIn}-${request.clockOut} Â· ${request.reason}`;
...
function renderEmployeeRequests() {
  ...
  $('#myTimeEditRows').innerHTML = myEdits.map(request => `<div class="approval-row"><div><b>${request.date}</b><small>${request.clockIn}-${request.clockOut} Â· ${escapeHtml(request.reason)}</small></div><span class="access-state ${request.status === 'pending' ? 'pending' : ''}">${request.status}</span></div>`).join('') || '<div class="empty-state">No time edit requests yet.</div>';
  renderLeaveCalendar();
}

function renderDocuments() {
  const mine = coachingDocuments.filter(document => document.employeeEmail === currentAccount?.email);
  $('#documentRows').innerHTML = mine.map(document => `<div class="document-row"><div><b>${escapeHtml(document.name)}</b><small>${document.size} Â· uploaded ${document.uploadedAt}</small></div><span class="access-state">Saved</span></div>`).join('') || '<div class="empty-state">No coaching documents uploaded yet.</div>';
}
...
function openEmployeeDetail(name) {
  ...
  const person = liveEmployees().find(item => item.name === name) || allRosterWithExtras().find(item => item.name === name) || (added && { name: added.name, role: added.jobRole, department: added.department || 'Added by admin', task: 'Not assigned', rate: null, scheduledStart: null, scheduledEnd: null, schedule: 'Not assigned', initials: added.initials, color: '#bad7ed', clockIn: 'â€”', clockOut: 'â€”', worked: '0h 00m', status: 'complete' });
  if (!person) return;
  ...
  $('#employeeDetailRole').textContent = `${person.role} Â· ${person.department}`;
...
$('#loginForm').onsubmit = async event => {
  ...
  setLoginMessage('Signing inâ€¦ please wait.', false);
  try {
    if (supabaseClient) {
...
async function createDeletedTimeNotice({ employeeId, employeeName, timeLabel, reason }) {
  ...
  if (usesSupabase()) {
    const { error } = await supabaseClient.from('app_notifications').insert({
      ...
      title: notice.title,
      message,
      meta: { timeLabel, reason: reason || '', deletedBy: currentProfile?.id || null }
    });
    if (error) {
...
async function deleteEmployeeTimeEntry(button) {
  ...
  if (usesSupabase()) {
    ...
    await createDeletedTimeNotice({ employeeId, employeeName, timeLabel, reason });
    await loadSupabaseTimeEntries();
  } else {
...
$('#holidayCalendar').onclick = event => {
  ...
  showToast(holiday ? `${holiday.name}: ${holiday.type === 'regular' ? '2Ã— regular-holiday pay applies.' : 'special non-working day.'}` : `${new Date(day.dataset.calendarDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric' })}: no listed holiday.`);
};
