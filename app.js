...
const employeeDirectory = rosterRows.map(([name, role, department, task, rate, scheduledStart, scheduledEnd, tempPassword], index) => ({
  ...
  date: 'Today',
  status: index < 4 ? 'clocked' : 'complete'
}));
...
function renderAttendance() {
  ...
  const rows = employeeDirectory.filter(person => (!term || person.name.toLowerCase().includes(term) || person.role.toLowerCase().includes(term)) && (filter === 'all' || person.status === filter));
  ...
  $('#clockedInCount').textContent = employeeDirectory.filter(person => person.status === 'clocked').length;
}

function renderLiveTeam() {
  const online = employeeDirectory.filter(person => person.status === 'clocked');
...
function renderScheduleWatch() {
  const statuses = employeeDirectory.map(person => ({ person, status: scheduleStatus(person) }));
  $('#checkedInMetric').textContent = employeeDirectory.filter(person => person.status === 'clocked').length;
  ...
  const live = employeeDirectory.filter(person => person.status === 'clocked');
...
function openEmployeeDetail(name) {
  ...
  const person = allRosterWithExtras().find(item => item.name === name) || (added && { name: added.name, role: added.jobRole, department: added.department || 'Added by admin', task: 'Not assigned', rate: null, scheduledStart: null, scheduledEnd: null, schedule: 'Not assigned', initials: added.initials, color: '#bad7ed', clockIn: '—', clockOut: '—', worked: '0h 00m', status: 'complete' });
  if (!person) return;
...
(async function init() {
  ...
  setInterval(renderScheduleWatch, 1000);
})();
