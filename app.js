...
function buildPayrollRows() {
  ...
  return rosterSource().filter(person => payrollRole(person) === selectedPayrollRole).map(person => {
...
function renderPayroll() {
  ...
  $('#payrollRows').innerHTML = currentPayrollRows.map(row => {
    const person = `<div class="person"><span class="person-avatar" style="background:${row.person.color}">${row.person.initials}</span><span>${escapeHtml(row.person.name)}<small>${escapeHtml(row.person.role)}</small></span></div>`;
    const edit = `<span class="payroll-actions"><button class="edit-adjustment-btn" data-payroll-edit="${row.person.id}">Edit</button><button class="paystub-btn" data-paystub="${row.person.id}">Paystub</button></span>`;
...
async function downloadEmployeePaystub(employeeId) {
  ...
  doc.save(`Sync2Time-Paystub-${slug(row.person.name)}-${isoDate(start)}-to-${isoDate(end)}.pdf`);
...
document.body.addEventListener('click', async event => {
  ...
  if (paystub) {
    event.stopPropagation();
    await downloadEmployeePaystub(paystub.dataset.paystub);
    return;
  }
