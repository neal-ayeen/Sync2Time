...
function renderTeamDirectory() {
  ...
  $('#teamRows').innerHTML = roster.map(person => {
    ...
    const hasSupabaseAccess = !!(usesSupabase() && person.id && supabaseProfiles.some(profile => profile.id === person.id));
    ...
    const action = `<span class="team-actions">${clockOutAction}${clearAction}</span>`;
    const accessActive = !!(access || hasSupabaseAccess);
    return `<div class="team-row" role="button" tabindex="0" data-employee="${encodeURIComponent(person.name)}"><div class="person"><span class="person-avatar" style="background:${person.color}">${person.initials}</span><span>${person.name}${passwordLine}</span></div><span>${person.role}<small>${person.department || ''}</small></span><span class="schedule-shift">${person.schedule || 'Not assigned'}</span><span class="schedule-shift">${typeof person.rate === 'number' ? rateLabel(person) : person.rate || 'Pending'}</span><span class="access-state ${accessActive ? '' : 'pending'}">${accessActive ? 'Access active' : 'No access'}</span><span>${action}</span></div>`;
  }).join('');
...
function approvalRow(request, type) {
  const details = type === 'leave'
    ? `<small>${escapeHtml(request.date)}</small><small>${escapeHtml(request.reason)}</small>`
    : type === 'overtime'
      ? `<small>${escapeHtml(request.date)}</small><small>${Number(request.hours).toFixed(2)} OT hours</small><small>${escapeHtml(request.reason)}</small>`
      : `<small>${escapeHtml(request.date)}</small><small>${escapeHtml(request.clockIn)}-${escapeHtml(request.clockOut)}</small>`;
  ...
  return `<div class="approval-row"><div><b>${escapeHtml(request.employeeName)}</b>${details}</div><span class="access-state ${request.status === 'pending' ? 'pending' : ''}">${request.status}</span>${actions}</div>`;
}
...
function payrollRange() {
  ...
  if ($('#payrollCutoff')?.value === 'previous') {
    if (reference.getDate() <= 15) reference.setMonth(reference.getMonth() - 1, 16);
    else reference.setDate(1);
  }
  let start;
...
function scheduledHoursInRange(person, start, end) {
  ...
  while (cursor <= last) {
    if (cursor.getDay() !== 0 && cursor.getDay() !== 6) workdays++;
...
function payrollUsdPhpRate() {
  const manual = Number(localStorage.getItem('sync2time-payroll-usd-php'));
  return manual > 0 ? manual : (liveUsdPhpRate || 1 / phpUsdRate);
}
...
function renderPayroll() {
  ...
  const manual = Number(localStorage.getItem('sync2time-payroll-usd-php')) > 0;
  ...
  $('#payrollRangeLabel').textContent = `${start.toLocaleDateString()} to ${end.toLocaleDateString()} · ${selectedPayrollRole.toUpperCase()}`;
...
async function buildEmployeePaystub(employeeId, shouldDownload = true) {
  ...
  const { start, end } = payrollRange();
  const payDate = end.getDate() === 15
    ? new Date(end.getFullYear(), end.getMonth(), 20)
  ...
  doc.text(`Pay date: ${payDate.toLocaleDateString('en-US')}`, 145, 90);
  ...
  doc.text(`Payroll cutoff: ${start.toLocaleDateString('en-US')} - ${end.toLocaleDateString('en-US')}`, 38, 199);
  ...
  doc.setFontSize(8);
  doc.text(`Generated by Sync2Time on ${new Date().toLocaleString('en-US')}`, 38, 808);
...
function openPayrollRowEditor(employeeId) {
  ...
  $('#payrollRowPeriod').textContent = `${start.toLocaleDateString()} to ${end.toLocaleDateString()} · ${row.person.role}`;
...
$('#employeeImport').onchange = event => {
  ...
  reader.onload = async () => {
    ...
    for (const [name, email, jobRole, password] of rows) {
      ...
      managedEmployees.push({ id: crypto.randomUUID(), name, email: email.toLowerCase(), jobRole: jobRole || 'Employee', department: 'Imported', initials: initials(name), tempPassword: password, hash: await hashPassword(password), rate: null, rateType: '', scheduledStart: null, scheduledEnd: null, paystubEmail: email.toLowerCase() });
      added++;
    }
    persistEmployees();
    renderManagedEmployees();
...
$('#editEmployee').onclick = () => {
  const employee = editableEmployeeRecords().find(item => item.id === $('#manageEmployeeSelect').value);
...
$('#deleteEmployee').onclick = () => {
  ...
  if (!employee) return showToast('This employee is synced from Supabase. Keep the history safe and delete the live account manually in Supabase for now.');
...
['liveTeamRows', 'scheduleRows', 'projectRows', 'teamRows', 'attendanceRows'].forEach(id => {
  $(`#${id}`).onclick = event => {
    if (event.target.closest('[data-delete-time], [data-admin-clockout], [data-clear-employee-time]')) return;
