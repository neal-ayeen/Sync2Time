...
async function loadPaystubRecipients() {
  ...
  if (!error) paystubRecipients = data || [];
}
...
function renderPayroll() {
  ...
  $('#payrollRows').innerHTML = currentPayrollRows.map(row => {
    ...
    const edit = `<span class="payroll-actions"><button class="approve-paystub-btn ${row.paystubApproved ? 'approved' : ''}" data-approve-paystub="${row.person.id}" ${recipient ? '' : 'disabled title="No paystub recipient"'}>${approvalLabel}</button><button class="edit-adjustment-btn" data-payroll-edit="${row.person.id}">Edit</button><button class="paystub-btn" data-paystub="${row.person.id}">Paystub</button></span>`;
...
async function sendApprovedPaystubs() {
  const rows = currentPayrollRows.filter(row => row.paystubApproved);
  const allReady = rows.length === currentPayrollRows.length && rows.every(row => paystubRecipients.some(item => item.employee_id === row.person.id));
  if (!allReady || !rows.length) return showToast('Approve every employee and assign every recipient first.');
