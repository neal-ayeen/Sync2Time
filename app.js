...
const SUPABASE_URL = 'https://zbhtsroeotezfmfxeaus.supabase.co';
...
function renderAttendance() {
  ...
  const source = filter === 'clocked' ? live : [...live, ...rosterSource().filter(person => !liveEmails.has(person.email))];
...
async function applySupabaseSession(session) {
  ...
  if (error || !data) {
    $('#loginError').textContent = 'Login worked, but this user does not have a Sync2Time profile yet.';
...
$('#toggleTimer').onclick = async () => {
  ...
  if (state.running) {
    ...
    state.running = { task, start, timeEntryId };
...
$('#loginForm').onsubmit = async event => {
  ...
  if (supabaseClient) {
    ...
    if (!error && data.session) {
      ...
      showToast(`Welcome back, ${currentAccount.name}.`);
      return;
    }
    if (error) showToast(`Supabase login: ${error.message}`);
  }
